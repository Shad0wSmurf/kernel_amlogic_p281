...
static void spdifin_status_event(struct aml_spdif *p_spdif)
{
	...
	if (intrpt_status & 0x4) {
		...
		if (sample_last_mode == mode)
			return;
		sample_last_mode = mode;
		/* pr_info("Notify new sample mode:0x%x\n", mode); */
			       ...
			       struct snd_soc_dai *cpu_dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		/* force clean sample rate irq bit for axg */
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE
			&& p_spdif->chipinfo
			&& p_spdif->chipinfo->clr_irq_all_bits)
			aml_spdifin_clr_irq(p_spdif->actrl,
